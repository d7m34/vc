...
/*
 * Discord Selfbot Voice Recorder
 * ================================
 * يستخدم discord.js-selfbot-v13 للانضمام للقنوات الصوتية
 * وتسجيل جميع المتكلمين في ملفات PCM ثم تحويلها لـ MP3
 *
 * الطريقة الصحيحة في discord.js-selfbot-v13:
 *   - channel.join() → يعيد VoiceConnection
 *   - connection.receiver → يعطي VoiceReceiver
 *   - receiver.createStream(userId, { mode: 'pcm', end: 'manual' }) → ReadableStream
 *   - receiver يطلق event اسمه 'speaking' عندما يبدأ شخص بالكلام
 */
...
if (!TOKEN || !WEBHOOK_URL) {
  console.error('[FATAL] TOKEN and WEBHOOK_URL environment variables are required.');
  process.exit(1);
}

// ─── ثوابت ──────────────────────────────────────────────────────
const TEMP_DIR = path.join(__dirname, 'temp');
const MAX_PCM_BYTES = 20 * 1024 * 1024; // 20 MB
const ROTATE_CHECK_MS = 1000; // فحص كل ثانية
